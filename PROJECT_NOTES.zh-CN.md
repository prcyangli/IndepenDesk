...
更新日期：2026-09-22（v0.4.16）

本文记录基于 `main` 分支（原始基线提交 `21c9b74`）完成的本地功能修改、代码审查结论、本轮修复状态和验证清单。

## v0.4.16：睡眠/唤醒时保留外接显示器的桌面分组；内屏固定编号 1（2026-09-22）

- **内屏固定编号**：`OrderedMonitors` 改为内屏优先、外接屏按屏幕位置跟随；显示器序号与 `Ctrl+Alt+1..9` 全局桌面号共用该顺序（内屏桌面占最小编号）。`GetLayout` 将呈现顺序与编号顺序解耦：总览页外接屏行在前、内屏行固定最后，右键移动菜单同序。台式机（无内屏）回退原从左到右编号；内屏离线时沿用最后一次拓扑快照的 `IsInternal` 身份（`LastDisplay`）。8 语言 README 的编号说明同步更新；Release 编译 0 警告 0 错误。
