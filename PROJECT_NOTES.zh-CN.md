...
- "窗口被跳过"气泡每会话最多一条，托盘菜单可关闭（`settings.json` 键 `warnUnmanageable`，默认开）；8 种语言新增 `msg.windowSkipped` / `menu.warnUnmanageable`。
- 修复末尾无限新建：不可管理窗口始终跟随当前桌面，此前按 → 时会因其"非空"而不断新建。`SwitchRelative` 的 canGrow 改为"存在有效窗口"（存活句柄且不在跳过集合），仅剩不可管理窗口时与空桌面同规则（只弹 OSD）；总览显式 "＋" 新建不受影响。
- `Ctrl+Alt+Shift+←/→` 移动活动窗口：活动窗口不可管理时不移动、不新建桌面，复用跳过通知。
