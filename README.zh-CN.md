...
可选的**共用任务栏**模式（托盘菜单）不再隐藏非当前桌面的窗口，而是把它们停靠到屏幕外。所有桌面的窗口因此保留在任务栏和 Alt-Tab 中，激活其中任何一个都会跳转到它所在的桌面。停靠位置与隐藏状态一样记录到磁盘日志，崩溃后重启会自动还原。
...
- 共用任务栏模式下，停靠的窗口仍在渲染（GPU/CPU 占用略高于隐藏模式）。若该模式下进程被强制结束，请再次启动 IndepenDesk 0.4.4 或更高版本以将停靠窗口恢复到屏幕内（旧版本无法读取 v3 日志格式）。
